			...
			if (str[0] == '?') {
				...
				for(i = 1; i < len; i++){
					if(isdigit((int)str[i])>0){
						...
						if(str[k+1] != '\n'){
							check = 0;
							printf("no new line after number\n");
							break;
						}
///I DONT KNOW HOW TO USE THE NEWLINE THING. I NEED IT TO GET THE INFORMATION AFTER A NEWLINE BUT IT ENDS THIS FUNCTION. DO I CALL FGETS AGAIN??

						else{
							char message[len-(j+i)];
							memcpy(message, &str[j+i], len-(j+i));
							printf("the update message is %s \n",message);
						}
